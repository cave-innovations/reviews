...
function populateReview(review) {
  const reviewSql = `insert into reviews (traveler_firstname,traveler_lastname, date, image_url, review, rating_accuracy, rating_communication, rating_cleanliness, rating_location, rating_checkin, rating_value) values('${review.traveler_firstName}','${review.traveler_lastName}', '${review.date.toJSON().slice(0, 10).replace('T', ' ')}', '${review.imageUrl}', '${review.review}', ${review.rating_accuracy}, ${review.rating_communication}, ${review.rating_cleanliness}, ${review.rating_location}, ${review.rating_checkin}, ${review.rating_value})`
...
function populateResponse(response) {
  const responseSql = `insert into responses (date, response) values(
  '${response.date.toJSON().slice(0, 10).replace('T', ' ')}', '${response.response}')`
  db.query(responseSql, (err, rows, fields) => {
    if(err) {
      console.log(err)
    } else {
      console.log(`Response to the review posted!`)
    }
  })
}
